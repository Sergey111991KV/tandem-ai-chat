...
3. Configure TDLib native libs (Telegram `apiId`/`apiHash` already match iOS Release in
   `app/assets/config/app_config.json`; override locally if needed):

```bash
cp config/app_config.example.json config/app_config.local.json   # optional override
